...
export default function MessageList({messages, currentUser, theme, isGroupChat, pars, usersInfo }) {
    ...
    // Function to determine if username should be shown
    const shouldShowUsername = (message, index) => {
        if (!isGroupChat || message.sender === currentUser.id) {
            return false;
        }

        // Show username if this is the first message
        if (index === 0) {
            return true;
        }

        // Show username only if the previous message was from a different sender
                        ...
                        <MessageItem
                            ...
                            senderName={
                                shouldShowUsername(message, index) ?
                                  pars.find(p => p.user_id === message.sender)?.alias || message.sender : null
                            }
                            senderProfilePic={
                                shouldShowUsername(message, index) ?
                                    usersInfo.find(user => user.id === message.sender)?.profile_pic || null : null
                            }
                        />
                    </Pressable>
                ))}
